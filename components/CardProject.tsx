...
export default function CardProject({ project }: CardProjectProps) {
    return (
        <div
            className="w-full md:w-[90%] h-auto mx-auto p-[2px] rounded-md mb-[4%] bg-gradient-to-b from-[#41d1ff] to-[#9231be] cursor-pointer hover:scale-[1.025] transition-all duration-300 ease-in-out"
        >
            <div className="bg-[#18181B] p-4 rounded-md w-full h-full relative">

                {/* If its topStart */}
                {/* {project.topStart && (
                    <Image src={'star.webp'} customClass="w-7 absolute top-2 right-2" />
                )} */}
                ...
                <div className="mt-[4%] w-full h-auto">
                    <a className="flex items-center justify-start gap-2 mt-2 group" href={project.github} target='_blank'>
                        <Image src={'github.webp'} customClass="w-[1.5rem] transition-all group-hover:rotate-[45deg]" />
                    ...
                    <div className="flex items-center flex-wrap gap-2 mt-2">
                        ...
                        {project.tecnhnologies.map((tech, index) => (
                            <div key={index} className="flex items-center justify-start relative group">
                                <Image src={`${tech.toLowerCase()}.webp`} customClass="w-[1.8rem]" />
